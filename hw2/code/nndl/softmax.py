...
class Softmax(object):
  ...
  def loss(self, X, y):
    ...
    for i in range(X.shape[0]):
      ...
      loss += -scores[y[i]] + np.log(np.sum(list(map(func1,scores))))
    loss /= X.shape[0]
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
  ...
  def loss_and_grad(self, X, y):
    ...
    for i in range(X.shape[0]):
      ...
      for j in range(self.W.shape[0]):
        grad[j] += np.exp(scores[j])/np.sum(list(map(func1,scores))) * X[i]
        if j == y[i]:
          grad[j] -= X[i]
    loss /= X.shape[0]
    grad /= X.shape[0]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    scores_log = np.log(np.sum(scores_exp,axis = 1))
    loss = scores_log - scores[np.arange(X.shape[0]),y]
    loss = np.mean(loss)

    scores_div = scores_exp / np.sum(scores_exp,axis = 1).reshape(scores_exp.shape[0],1)
