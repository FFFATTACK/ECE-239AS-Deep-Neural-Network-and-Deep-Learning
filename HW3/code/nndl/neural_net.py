...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores = np.maximum(0,(X.dot(W1.T) + b1)).dot(W2.T) + b2    
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    loss_array = -np.log(softmax[range(N),y])
    softmax_loss = np.sum(loss_array) / N
    l2 = reg * 0.5 * np.sum(W1 * W1) + 0.5 * reg * np.sum(W2 * W2)
    loss = softmax_loss + l2
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    # ================================================================ #
    hidden = np.maximum(0,X.dot(W1.T) + b1)
    dsoftmax = softmax
    dsoftmax[range(N),y] -= 1
    dsoftmax /= N
    grads["W2"] = dsoftmax.T.dot(hidden)
    grads["b2"] = np.sum(dsoftmax,axis = 0)

    dsoftmax = dsoftmax.dot(W2)  
    dsoftmax[hidden <= 0] = 0
    grads["W1"] = dsoftmax.T.dot(X)
    grads["b1"] = np.sum(dsoftmax,axis = 0)
